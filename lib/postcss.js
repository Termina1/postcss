...
class PostCSS {
    ...
    async(css, opts = {}) {
        ...
        var parsed = new Promise((resolve) => {
            resolve(this.getParsed(css, opts));
        });
        return Promise.reduce(this.processors, (parsed, processor) => {
            var result;
            if(processor.length > 2) {
                result = new Promise(processor.bind(null, parsed, postcss));
            } else {
                result = Promise.resolve(processor(parsed, opts));
            }
            return result.then((returned) =>
                returned instanceof Root ? returned : parsed);
        }, parsed).then(parsed => parsed.toResult(opts));
    }
    ...
    process(css, opts = { }) {
        ...
        var parsed = this.getParsed(css, opts);
